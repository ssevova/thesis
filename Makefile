...
example.pdf: example.tex preamble.tex chap1-dmtheory.tex chap2-cms.tex chap3.tex frontmatter.tex appendices.tex
#	@rm -f $(EXTRASTYS)
#	unzip extrastyles.zip
	@rm -f example.{aux,toc,lof,lot}
#	pdflatex example && bibtex example && pdflatex example && pdflatex example
	lualatex example && bibtex example && lualatex example && lualatex example
	@rm -f example.{aux,toc,lof,lot}
#	@rm -f $(EXTRASTYS)
